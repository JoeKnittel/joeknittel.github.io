...
2. Direct the `BinderHub` to your repository (and, specifically, the notebook), so a `container` (an immutable, ready-to-run software package) can be built and stored in a registry (specifically, that of <a href = "https://cloud.google.com/container-registry/">Google Cloud</a>) for future processing.
